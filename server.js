...
const store = new DocumentStore("http://live-test.ravendb.net", "test");
store.initialize();
...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    if (existingUser > 0) {
      return res
        .status(400)
        .send({ message: "Username already exists, please use another" });
    }
...
app.get("/products", async (req, res) => {
  const session = store.openSession();
  try {
    let query = await session.query({ collection: "Products" });

    // Existing filters
    if (req.query.search) {
      query = query.whereEquals("name", req.query.search);
    }
    if (req.query.itemType) {
      query = query.whereEquals("itemType", req.query.itemType);
    }
    if (req.query.category) {
      query = query.whereEquals("category", req.query.category);
    }

    // New country filter
    if (req.query.country) {
      query = query.whereEquals("country", req.query.country);
    }
...
app.post("/send-whatsapp-message", async (req, res) => {
  try {
    ...
    const whatsappMessage = await client.messages.create({
      body: message,
      from: "whatsapp:", //add yourphone number with country code
      to: `whatsapp:${phone}`,
    });
